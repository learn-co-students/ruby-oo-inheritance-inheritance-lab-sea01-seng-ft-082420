...
[x]1. Define the `User` class such that a user can have a first and last name. You'll need both a setter and a getter for first and last name. 
...
[x]3. We've given you a class constant `KNOWLEDGE`, that points to an array of knowledge strings. Write a method, `#teach` that returns a random element from that array. You might want to look up the `Array#sample` method. 
...
[x]5. Individual students should initialize with an instance variable, `@knowledge`, that points to an empty array. 
[x]6. Define a method, `#learn`, that takes in a string and adds that string to the student's `@knowledge` array. 
[x]7. Define a method, `#knowledge`, that returns that student's knowledge array. 
